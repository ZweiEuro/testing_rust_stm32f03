use stm32f0xx_hal::{pac::TIM16, stm32f0, timers::Timer};
...
pub fn clear_irq_tim16() {
    cortex_m::interrupt::free(move |_cs| unsafe {
...
pub fn setup_1ns_timer() -> () {
    if let (Some(mut p), Some(cp)) = (Peripherals::take(), c_m_Peripherals::take()) {
        ...
        panic!("Error: Could not config timer and LED");
    }
}
